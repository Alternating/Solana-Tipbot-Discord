...
exports.connect = function() {
  try {
    mongoose.connect('mongodb://localhost/tetsuo',
        {
...
exports.checkProfileInMission = async function(discordID, missionID) {
  try {
    let profile = await Profile.findOne({ userID: discordID });
    let mission = await Mission.findOne({ missionID: missionID });

    for (var i = 0; i < mission.profiles.length; i++) {
      let missProfID = JSON.stringify(mission.profiles[i])
      let checkProfID = JSON.stringify(profile._id)
      if (missProfID == checkProfID) {
        return true
      }
    }
    return false
